...
#Out="results/VariantAnalysis_DNA/Synonyms_and_nonsynonymous_fromGTF/SND_positions.bed"
#OutCallab="results/VariantAnalysis_DNA/Synonyms_and_nonsynonymous_fromGTF/SND_positions_callable.bed"
...
cat ${GTF} | awk '{if($3 == "CDS"){print $0}}' | cut -f1,4,5,9 | sed 's/gene_id "\(\S\+\)";.*transcript_id "\(\S\+\)";.*/\1\t\2/g' | sort -k1,1 -k5,5 -k2,2V | awk -F'\t' '{if(t==$5){l+=$3-$2;next}else{print c,l,g,t;c=$1;l=$3-$2;g=$4;t=$5}}END{print c,l,g,t;}' | tail -n +2 | tr ' ' '\t' | sort -k1,1 -k3,3 -k2,2V | awk '{if(g==$3){t=$4}else{print g,t;g=$3;t=$4}}END{print g,t;}' | tail -n +2 | tr ' ' '\t' > $(dirname ${Out})/LonguestTranscriptPerGene2.tab

echo "----> Extract CDS bed of longest transcript per sequence"
awk -F'\t' '{if(NR==FNR){a[$1]=$2;next}if(a[$5]==$6){print $0}}' $(dirname ${Out})/LonguestTranscriptPerGene2.tab <(cat ${GTF} | awk '{if($3=="CDS"){print $0}}' | cut -f1,4,5,7,9 | sed 's/gene_id "\(\S\+\)";.*transcript_id "\(\S\+\)";.*/\1\t\2/g') | sort -k1,1 -k5,5 -k2,2V | awk '{print $1"\t"$2"\t"$3"\t"$4":"$5":"$6}' > $(dirname ${Out})/LonguestTranscript_CDS.bed

echo "----> Extract CDS sequence from genome"
bedtools getfasta -name -fi ${genome} -bed <(cat $(dirname ${Out})/LonguestTranscript_CDS.bed | awk '{st=$2-1; print $1"\t"st"\t"$3"\t"$4}') -bedOut | sed 's/:/\t/g' | awk '{st=$2+1; print $1"\t"st"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7}' > $(dirname ${Out})/CDS_exon_sequences.bed

echo "----> Join transcript sequence"
cat $(dirname ${Out})/CDS_exon_sequences.bed | awk '{if($5 == g){seq=seq$7}else{print g"\t"sense"\t"seq; g=$5; seq =$7; sense=$4}}END{print g"\t"sense"\t"seq;}' | tail -n +2 > $(dirname ${Out})/CompleteCDS_sequence.tab

echo "----> Get Syn and nonSyn positions from transcript sequences"
# check 5 transcript with N!!!
cat $(dirname ${Out})/CompleteCDS_sequence.tab | grep -v "N" | awk -v scr=${scriptSnSfromSeq} '{cmd=scr"\t"$3"\t"$2; cmd | getline nseq; close(cmd); print $1"\t"nseq}' > $(dirname ${Out})/SnS_CompleteCDS_sequence.tab
#cat $(dirname ${Out})/CompleteCDS_sequence.tab | grep -v "N" | awk -v scr=${scriptProtfromTransc} '{cmd=scr"\t"$3"\t"$2; cmd | getline nseq; close(cmd); print $1"\t"nseq}' > $(dirname ${Out})/Prot_sequence.tab


echo "----> Filter out SnS seuqences that are not the same lenght as the transcript"
## Step to be checked:
# differences in length come from errors in the transcript sequence
# N values within the trnascript or len%3!=0
